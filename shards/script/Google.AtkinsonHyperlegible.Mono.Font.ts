...
export default defineShard(async () => {
	const [head] = await githubClient.rest.repos
		.listCommits({ owner: 'googlefonts', repo: 'atkinson-hyperlegible-next-mono', per_page: 1 })
		.then(({ data }) => data);
	...
	const font = await ky(
		'https://raw.githubusercontent.com/googlefonts/atkinson-hyperlegible-next-mono/' +
			sha +
			'/fonts/otf/AtkinsonHyperlegibleMono-Regular.otf',
	).arrayBuffer();

	return {
		version: () => fontVersion(new Uint8Array(font)),
		urls: [
			'https://github.com/googlefonts/atkinson-hyperlegible-next-mono/archive/' + sha + '.zip',
		],
		state: sha,
	};
});
